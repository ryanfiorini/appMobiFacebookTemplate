    ...
    ui.prototype = {
        ...
        */
        ...
        updateNavbarElements: function(elems) {
            ...
            //IE is hanging onto the old DOM elements.
            nb.html("",true);
            ...
            if (this.firstDiv) {
                ...
                var loadFirstDiv=function(){
                   ...
                   if(jq("#navbar a").length>0){
                        jq("#navbar a").data("ignore-pressed", "true").data("resetHistory", "true");
                        that.defaultFooter = jq("#navbar").children().clone(true);
                        that.updateNavbarElements(that.defaultFooter);
                    }
					//setup initial menu
                    var firstMenu = jq("nav").get();
                    if(firstMenu){
                        that.defaultMenu = jq(firstMenu).children().clone(true);
                        that.updateSideMenu(that.defaultMenu);
                    }
					//get default header
                    that.defaultHeader = jq("#header").children().clone(true);
					//
                    jq("#navbar").on("click", "a", function(e) {
                        jq("#navbar a").not(e.target).removeClass("selected");
                    ...
                    if (firstPanelId.length > 0 && that.loadDefaultHash && firstPanelId!=("#"+that.firstDiv.id)) {
                        ...
                        $("#header #backButton").css("visibility","visible");
                    } else {
